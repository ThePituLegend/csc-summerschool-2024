...
void h5_writer(int my_id, int *localvector, int localsize)
{
    ...
    dset_id = H5Dcreate(file_id, "data", H5T_NATIVE_INT, filespace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    ...
    H5Dwrite(dset_id, H5T_NATIVE_INT, memspace, filespace, plist_id, localvector);
...
void h5_reader(int my_id, int *localvector, int localsize)
{
    ...
    plist_id = H5Pcreate(H5P_FILE_ACCESS);
    H5Pset_fapl_mpio(plist_id, MPI_COMM_WORLD, MPI_INFO_NULL);
    file_id = H5Fopen("parallel.h5", H5F_ACC_RDONLY, plist_id);
    H5Pclose(plist_id);
    ...
    plist_id = H5Pcreate(H5P_DATASET_XFER);
    H5Pset_dxpl_mpio(plist_id, H5FD_MPIO_COLLECTIVE);
    H5Dread(dset_id, H5T_NATIVE_INT, memspace, filespace, plist_id, localvector);
