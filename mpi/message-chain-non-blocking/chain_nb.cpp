...
int main(int argc, char *argv[])
{
    ...
    // TODO: Send messages
    MPI_Isend(message.data(), size, MPI_INT, destination, rank + 1, MPI_COMM_WORLD, &sendReq);

    printf("Sender: %d. Sent elements: %d. Tag: %d. Receiver: %d\n",
           rank, size, rank + 1, destination);

    // TODO: Receive messages
    MPI_Irecv(receiveBuffer.data(), size, MPI_INT, source, rank, MPI_COMM_WORLD, &recvReq);
